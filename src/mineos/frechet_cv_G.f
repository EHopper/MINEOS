...
c        
     ...
     &           + xnn*betah(j)**2.)
c
c		Z Eilon - swap azimuthal anis G kernel (same as L) for the Vsv column
c				  I think the normalisation factor (c/U)*Vsv is still the same
c            G velocity kernel
c
c            intg(j,3) = 2.*dnn(j)*betav(j)*(xll-2.*eta(j)*xff)
c	     intg_cad(j,3)=(cv/gv)*betav(j)*intg(j,3)
         intg_cad(j,3) = dnn(j)* betav(j)**2 *xll
c
c            PV kernel
c
