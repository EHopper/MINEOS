...
c                with, which might need to change over time.
c
c     01/29/17 - Swapped Vsv kernel for L kernel (which is the same as the G kernel)
c  				 for azimuthal anisotropy. Z. Eilon
c
...
c
c ========================================================================================
c     Modified by Z. Eilon, 28/09/17
c     Now this script produces the explicit kernels for the Love parameters
c     A,C,L,N,F (note order) in the place of Vsv, Vpv, Vsh, Vph, eta, rho
c     draw_frechet_gf, will then spit these kernels out in that order.
c     (at the moment it seems that draw_frechet_gv only outputs four columns, so 
c     something is awry there - that's why I shoved the F kernel at the end...)
c     
c     N.B. These kernels are NOT multiplied by the factor of rho*Vsv^2 as in Colleen's 
c     other codes
c ========================================================================================
...
c        
     ...
     &           + xnn*betah(j)**2.)
c
c		Z Eilon - swap A kernel for the SV column
c            A  kernel
c
c            intg(j,3) = 2.*dnn(j)*betav(j)*(xll-2.*eta(j)*xff)
c	     intg_cad(j,3)=(cv/gv)*betav(j)*intg(j,3)
         intg_cad(j,3) = xaa
c
c		Z Eilon - swap C kernel for the PV column
c
C            intg(j,4) = 2.*dnn(j)*alphav(j)*xcc
C     intg_cad(j,4)=(cv/gv)*alphav(j)*intg(j,4)
      intg_cad(j,4) = xcc
c
c		Z Eilon - swap L kernel for the SH column
c
C            intg(j,5) = 2.*dnn(j)*betah(j)*xnn
C     intg_cad(j,5)=(cv/gv)*betah(j)*intg(j,5)
         intg_cad(j,5) = xll
c
c		Z Eilon - swap N kernel for the PH column
c
C            intg(j,6) = 2.*dnn(j)*alphah(j)*(xaa + eta(j)*xff)
C     intg_cad(j,6)=(cv/gv)*alphah(j)*intg(j,6)
         intg_cad(j,6) = xnn
c
c		Z Eilon - swap F kernel for the eta column
c
             intg_cad(j,7) = xff
c
