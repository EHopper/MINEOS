...
c
c
      subroutine fix_class_k (b_file,icolor_fill)
c
...
c
            if ((im .le. 2).or.((ib.lt.3).and.(im.le.10))) then
              nnb(im,ib) = nn
              llb(im,ib) = ll
...
c
c             first order problem - decreasing frequency
c
              if ((wc - wl) .lt. 0) then
c              print*,' failed frequency test: ',ib,im,wc,wl,dw,dwlc
               nn = nt(il,ib+1)
               ll = lt(il,ib+1)
               wc = modes(inx(ll,nn),1)
               cc = modes(inx(ll,nn),3)
               gc = modes(inx(ll,nn),4)
               dw = abs(wc - wl)
               if ((abs(dw-dwl).gt.0.25).and.(abs(dw-dw2).gt.0.25)) then
                 nnb(im,ib) = maxn
                 llb(im,ib) = llb(im-1,ib) + 1
                 wl = wl + dwl
               else
                 ...
                 llb(im,ib) = ll
                 do ii = ib, nb - 1
                   if (index(lt(il, ii),nt(il, ii)) .eq. 6) then
                     nt(il, ii) = nt(il, ii + 1)
                     lt(il, ii) = il
                   endif
                 end do
...
c               print*,' failed frequency test: ',ib,im,wc,wl,dw,dwl
                nnb(im,ib) = maxn
                llb(im,ib) = llb(im-1,ib) + 1
                do ii = nb + 1, ib + 1, -1
                  i1 = ii - 1
